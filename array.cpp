...
int MinStep(int a[], int n)
{
    ...
    if(negative % 2 == 0)
    {
        // As count of negative is even so we will change all 0 to 1, total cost here will be count of 0s
        step = step + zero;
    }
    else
    {
        ...
        if(zero > 0)
        {
            // Change one zero to -1 and rest of them to 1
            // Total cost here will be count of '0'
            step = step + zero;
        }
        ...
        {
            // As no 0s are available so we have to change one -1 to 1 which will cost 2 to change -1 to 1
            step = step + 2;
        }
    }
